...
func (storer *StorerMongodb) Open() (err error) {
	...
	for i := 1; i <= storer.options.retryCount; i++ {
		...
		// connect fail
		log.Errorf("fail to connect to db: try %d err %v", i, err)
	...
	if storer.options.migrateUp == true {
...
//   - invoke "migrate -path user/deploy/migrations/ -database "mongodb://$IMS_SECURITY_SERVICE_AUTHEN_DATABASE_USER:$IMS_SECURITY_SERVICE_AUTHEN_DATABASE_PASSWORD@localhost:27017/imsSecurityAuthenDB" up" to migrate up
//   - invoke "migrate -path user/deploy/migrations/ -database "mongodb://$IMS_SECURITY_SERVICE_AUTHEN_DATABASE_USER:$IMS_SECURITY_SERVICE_AUTHEN_DATABASE_PASSWORD@localhost:27017/imsSecurityAuthenDB" down" to migrate down
//   - invoke "migrate -path user/deploy/migrations/ -database "mongodb://$IMS_SECURITY_SERVICE_AUTHEN_DATABASE_USER:$IMS_SECURITY_SERVICE_AUTHEN_DATABASE_PASSWORD@localhost:27017/imsSecurityAuthenDB" drop" to drop everything inside the database
// * may connect to mongodb server to check the database
//   - invoke "sudo apt install mongodb-clients" to install mongo client
//   - invoke "mongo localhost:27017/imsSecurityAuthenDB -u $IMS_SECURITY_SERVICE_AUTHEN_DATABASE_USER -p $IMS_SECURITY_SERVICE_AUTHEN_DATABASE_PASSWORD" to connect to server
//   - invoke "mongo localhost:27017/imsSecurityAuthenDB -u $IMS_SECURITY_SERVICE_AUTHEN_DATABASE_USER -p $IMS_SECURITY_SERVICE_AUTHEN_DATABASE_PASSWORD --eval "db.getCollectionNames()"" to show imsSecurityAuthenDB's all collections
//   - invoke "mongo localhost:27017/imsSecurityAuthenDB -u $IMS_SECURITY_SERVICE_AUTHEN_DATABASE_USER -p $IMS_SECURITY_SERVICE_AUTHEN_DATABASE_PASSWORD --eval "db.privilegeProfiles.find()"" to list all documents in collection privilegeProfiles
//
