...
func filterUsername(username string) bson.M {
	return bson.M{"username": username}
}

// get mongodb filter that specify name
func filterName(name string) bson.M {
...
// mongodb set specified password
func updateSetPassword(password []byte) bson.D {
	return bson.D{primitive.E{Key: "$set", Value: bson.D{primitive.E{Key: "password", Value: password}}}}
}
