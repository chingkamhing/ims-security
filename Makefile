...
proto:
	...
	protoc -I=./api/pb --micro_out=${MODIFY},paths=source_relative:${PROTO_DIR}/model --go_out=${MODIFY}:${PROTO_DIR}/model model.proto && \
		...
		rm -rf ${PROTO_DIR}/model/creapptive
	# message
	protoc -I=./api/pb --micro_out=${MODIFY},paths=source_relative:${PROTO_DIR}/message --go_out=${MODIFY}:${PROTO_DIR}/message message.proto && \
		...
		rm -rf ${PROTO_DIR}/message/creapptive
	# apigateway
	...
	# gateway
	protoc -I=./api/pb -I=${GOPATH}/src/github.com/grpc-ecosystem/grpc-gateway/third_party/googleapis -I=${GOPATH}/src/github.com/grpc-ecosystem/grpc-gateway --micro_out=${MODIFY}:${PROTO_DIR}/gateway --go_out=${MODIFY}:${PROTO_DIR}/gateway apigateway.proto
	# authen
	protoc -I=./api/pb -I=${GOPATH}/src/github.com/grpc-ecosystem/grpc-gateway/third_party/googleapis --micro_out=${MODIFY}:${PROTO_DIR}/authen --go_out=${MODIFY}:${PROTO_DIR}/authen authen.proto
	# user
	protoc -I=./api/pb -I=${GOPATH}/src/github.com/grpc-ecosystem/grpc-gateway/third_party/googleapis --micro_out=${MODIFY}:${PROTO_DIR}/user --go_out=${MODIFY}:${PROTO_DIR}/user user.proto
