...
const LockReaderContextInfo = () => {
    ...
    const rows = Object.entries(key).reduce((lines, [key, value]) => {
        switch (key) {
            ...
            case 'holder':
                return lines.concat({ description: 'Holder', value: userObject[value] && userObject[value] && userObject[value].name})
            case 'validPeriod':
...
export default LockReaderContextInfo
